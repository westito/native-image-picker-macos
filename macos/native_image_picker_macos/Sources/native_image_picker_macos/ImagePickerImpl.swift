...
class ImagePickerImpl: NSObject, ImagePickerApi {
    ...
    private func showPHPicker(
        _ picker: PHPickerViewController, noActiveWindow: @escaping () -> Void
    ) {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        Task {
            for result in results {
                ...
                let canLoadImage = itemProvider.hasItemConformingToTypeIdentifier(
                ...
                let canLoadVideo = itemProvider.hasItemConformingToTypeIdentifier(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            loadFileRepresentation(for: contentType) { (url, _, error) in
                if let error = error {
                    ...
                    continuation.resume(
                        throwing: NSError(domain: "INVALID_OBJECT", code: -1, userInfo: nil))
                }
            }
        }
    }
